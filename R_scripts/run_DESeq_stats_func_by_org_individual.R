# run_DESeq_stats_func_by_org_individual.R
# Created 6/22/16
...
setwd(wd_location)
# or
setwd("~/Desktop/Projects/Lab Stuff/Macaque project/func_output/individual_org_functions/R_torques/")
...
for (name in control_files) {
  control_names <- c(control_names, unlist(strsplit(name, split='_', fixed=TRUE))[6])}
...
for (name in exp_files) {
  exp_names <- c(exp_names, unlist(strsplit(name, split='_', fixed=TRUE))[6])}
...
for (x in exp_files) {
  ...
  if (y > 1) {
    ...
    temp_table <- temp_table[,c(2,3)]
    print (x)
    exp_table <- merge(exp_table, temp_table, by = "V3", all = T)  }
}
...
write.table(sorted_org_results, file = "DESeq_macaque_R_gnavus_functions.tsv", 
