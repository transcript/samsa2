...
control_files <- list.files(
  pattern = "*.tsv", full.names = T, recursive = FALSE)
control_names = ""
...
if (nrow(control_table_trimmed)>opt$number) {
...
# merging the tables
full_table <- control_table_trimmed_m
full_table_top30 <- control_table_filtered_m
